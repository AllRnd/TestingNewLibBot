...
#-m pip install python-telegram-bot nest-asyncio
...
CANCEL_KEYBOARD = ReplyKeyboardMarkup([["❌ Отменить"]], one_time_keyboard=True)

# Параметры для шкал
SCALE_PARAMS = {
    "🩺 SOFA": [
        ("PaO2/FiO2 (мм рт.ст.)", "pao2_fio2", float),
        ("Тромбоциты (×10³/мкл)", "platelets", float),
        ("Билирубин (мг/дл)", "bilirubin", float),
        ("Среднее АД (мм рт.ст.)", "map", float),
        ("Шкала Глазго (3-15)", "glasgow", int),
        ("Креатинин (мг/дл)", "creatinine", float),
        ("Диурез (мл/сут)", "urine_output", float)
    ],
    "💉 MODS": [
        ("PaO2/FiO2 (мм рт.ст.)", "pao2_fio2", float),
        ("Билирубин (мг/дл)", "bilirubin", float),
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    await update.message.reply_text(
        "🏥 <b>Медицинский бот-ассистент</b>\n"
        "📊 Выберите шкалу для оценки:",
...
async def select_scale(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if user_choice not in SCALE_PARAMS:
        await update.message.reply_text("❌ Неизвестная шкала")
        return ConversationHandler.END
...
async def handle_parameter(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    try:
        ...
        if current + 1 < len(params):
            ...
            await update.message.reply_text(f"➡️ Следующий параметр:\n{next_param}", reply_markup=CANCEL_KEYBOARD)
            return INPUT_PARAMS
        else:
            return await show_results(update, context)

    except ValueError:
        error_msg = "❌ Ошибка формата. Введите число"
...
async def show_results(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if scale == "🩺 SOFA":
        ...
        if score < 6:
            interpretation = "🟢 Низкий риск (&lt;10&#37; летальности)"
        elif 6 <= score < 12:
            interpretation = "🟡 Средний риск (15-20&#37; летальности)"
        else:
            interpretation = "🔴 Высокий риск (&gt;95&#37; летальности)"

    elif scale == "💉 MODS":
        score = calculate_mods(context.user_data)
        if score <= 4:
            interpretation = "🟢 Легкая дисфункция"
        elif 5 <= score <= 8:
            interpretation = "🟡 Умеренная полиорганная недостаточность"
        else:
            interpretation = "🔴 Тяжелая полиорганная недостаточность"
    ...
    result_message = (
        f"📊 <b>Результаты оценки по шкале {scale}</b>\n\n"
        f"⭐ Накоплено баллов: <b>{score}</b>\n"
...
async def cancel(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    await update.message.reply_text(
        "🚫 Текущая операция отменена. Для нового расчета нажмите /start",
        reply_markup=ReplyKeyboardRemove()
    )
    context.user_data.clear()
    return ConversationHandler.END
